...
freq(selects1971$ip4)

summary(schwarzen <- glm(data = selects1971, 
                         ip4 ~ classKriesi + age_cat + factor(sg1),
                         family = binomial(link = "probit")))

selects1971 %>% 
  group_by(classKriesi) %>% 
  count(ip4) %>% 
  drop_na() %>% 
  mutate(prop = n/sum(n)) %>% 
  filter(ip4 == 1) %>% 
  mutate(compmean = prop - 0.405)


selects1975 <- 
  selects %>% filter(year == 1975)

freq(selects1975$ip5)
