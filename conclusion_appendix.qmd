...
The crisis of the 90s did not mark a break-up of the Swiss dominant social bloc, but rather a re-composition and a change in the power balance within the bloc itself. While the crisis did not destabilized the left bloc that much, the right bloc, that I named "Liberal-Conservative" throughout this work, underwent important process of change.
...
modelsummary(list("PS" = reg_ps, "Greens" = reg_greens, "No party" = reg_abstention), stars = TRUE, coef_omit = c(1, 15:80), output = "gt")
...
modelsummary(list("PRD-PLR" = reg_plr, "PDC/PCS" = reg_pdc, "UDC" = reg_udc), stars = TRUE, coef_omit = c(1,15:80), output = "gt")
...
modelsummary(list("Left" = reg_left, "Right" = reg_right, "No leaning" = reg_center), stars = TRUE, coef_omit = c(1,15:80), output = "gt")
...
modelsummary(list("Redistribution" = reg_redistr, "Equal R&C CH-foreign" = reg_equal_rights_chforeign, 
       "State intervention" = reg_state), stars = TRUE, coef_omit = c(1,15:80), output = "gt")
```
...
selects1999_classifLatest_rec %>% 
  ...
  labs(caption = "Data source: SELECTS 1999")
...
```{r eu simple regression}
#| echo: false
#| message: false
#| warning: false
#| tbl-cap: "Binary logistic regression: support for EU integration. Log(odds). Data source: SELECTS 1999"
#| label: tbl-eu

selects1999 <- read_sav("data/selects1999_classifLatest_rec.sav")

selects1999 <- 
  selects1999 %>% 
  mutate(for_eu_int = if_else(op_eu == 1, 1, if_else(is.na(op_eu), NA, 0)))


regeu <- glm(data = selects1999, for_eu_int ~ income + education+as_character(sex) + as_character(language) + symp_udc + symp_ps + symp_prd)

modelsummary(list("For EU integration" = regeu), stars = TRUE, coef_omit = "Intercept", output = "gt")
```
