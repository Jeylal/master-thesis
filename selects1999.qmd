...
cortest <- 
  selects1999 %>% 
  dplyr::select(state_vs_market, join_eu, lower_t_hi, for_soc_exp, uneq_chance_for, env_vs_growth, quest_trad, again_nucl, income_hh, educ, lr1) %>% 
...
acm <- 
selects1999 %>% 
  dplyr::select(state_vs_market, join_eu, lower_t_hi, for_soc_exp, uneq_chance_for, env_vs_growth, quest_trad, again_nucl, sex, income_hh, educ, religion, sg1, class8) %>% 
  as_character() %>% 
  as_factor() %>% 
...
```{r}
summary(ordreg1 <- clm(data = selects1999, as_factor(join_eu) ~ income_hh + educ + state_vs_market + as_character(sex) + as_character(sc7c), Hess = TRUE))
```
