...
vox$age_cat <- cut(vox$age, c(13, 25, 45, 65, 99), right = FALSE, include.lowest = TRUE)

vox$annee_cat <- cut(vox$annee, seq(from = 1981, to = 2016, by = 2), right = FALSE, include.lowest = TRUE)
vox$annee_f <- factor(vox$annee)
...
summary(glmer_pss <- glmer(data = vox,
                           formula = pss ~ income + educ + age_cat + (income + educ | annee_cat/canton_f),
