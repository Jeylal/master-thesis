...
However, the extent to which this DBS can truly be described as bourgeois must be questioned. In the sense given by Swiss social scientists, the bourgeois bloc refers to an alliance of right-wing parties with close ties with business interests. For @amable_illusion_2017 the bourgeois bloc is an alliance of social groups whose characteristic feature is that they are the most advantaged in terms of, for instance, income and education. I argue that, in the Swiss context, the use of the term bourgeois bloc can be misleading, because this bloc gathers, of course, parts of the wealthiest Swiss social groups, but also parts of popular groups, mainly through the Christian Democrats. 
...
The transformation of party systems into a mutli-elite and tripolar political space came with important change in the structural basis of party support. @oesch_rennwald_class_2010 show that, in Switzerland, the cultural cleavage is still rooted in the class structure, but caused a re-alignment between class and party support. Before this shift took place, the Swiss Socialist party was a hybrid-class party gathering above-average support from socio-cultural professionals (teachers, academics, journalists...) and production workers, but also from technicians, clerks and service workers [@rennwald_social_2020]. This strong class heterogeneity of the Swiss left declined: an analysis of Swiss post-election surveys for 2003 and 2007 show that the Swiss left was deserted by production workers who directed their support for the Swiss People's party due to cultural issues [@oesch_rennwald_class_2010]. Using European Social Survey data for 2011 and 2015, @rennwald_social_2020 show that socio-cultural professionals remain the only "party preserve" of the Swiss left.
...
Although the results presented by Oesch and Rennwald offer interesting and well documented understanding of the transformation of party support's class structure in Switzerland, their work is limited by some methodological and theoretical flaws. First, their definition of social class, based on @oesch
...
The main dependent variables are binary variables for party support. 
...
glmer_ps <- glmer(data = vox, 
                          pss ~ incomef + educationf + age_cat + gender + religion + house_ownership + (1 | annee_f) + (1 | canton),
                          family = binomial(link = "probit"),
                          nAGQ = 0, control = glmerControl(calc.derivs = FALSE, optimizer = "nloptwrap"))
glmer_ps2 <- glmer(data = vox, 
                          pss ~ incomef + educationf + age_cat + gender + religion + house_ownership + (incomef + educationf | annee_f) + (incomef + educationf | canton),
                          family = binomial(link = "probit"),
                          nAGQ = 0, control = glmerControl(calc.derivs = FALSE, optimizer = "nloptwrap")) 

glmer_udc3 <- glmer(data = vox, 
                          udc ~ incomef + educationf + age_cat + gender + religion + house_ownership + scale(share_50perc_nationalincome) + scale(share_50perc_nationalincome) + (incomef + educationf | annee_f) + (incomef + educationf | canton),
                          family = binomial(link = "probit"),
                          nAGQ = 0, control = glmerControl(calc.derivs = FALSE, optimizer = "nloptwrap"))

glmer_plr <- glmer(data = vox, 
                          plr ~ incomef + educationf + age_cat + gender + religion + house_ownership + (1 | annee_f) + (1 | canton),
                          family = binomial(link = "probit"),
                          nAGQ = 0, control = glmerControl(calc.derivs = FALSE, optimizer = "nloptwrap"))

glmer_pdc <- glmer(data = vox, 
                          pdc_pcs ~ incomef + educationf + age_cat + gender + religion + house_ownership + (1 | annee_f) + (1 | canton),
                          family = binomial(link = "probit"),
                          nAGQ = 0, control = glmerControl(calc.derivs = FALSE, optimizer = "nloptwrap"))

glmer_greens <- glmer(data = vox, 
                          green ~ incomef + educationf + age_cat + gender + religion + house_ownership + (1 | annee_f) + (1 | canton),
                          family = binomial(link = "probit"),
                          nAGQ = 0, control = glmerControl(calc.derivs = FALSE, optimizer = "nloptwrap"))

glmer_noparty <- glmer(data = vox, 
                          no_party ~ incomef + educationf + age_cat + gender + religion + house_ownership + (1 | annee_f) + (1 | canton),
                          family = binomial(link = "probit"),
                          nAGQ = 0, control = glmerControl(calc.derivs = FALSE, optimizer = "nloptwrap"))
...
glmer_ps %>% 
  avg_slopes()
r2_nakagawa(glmer_ps, tolerance = 1e-1000)

library(VGAM)
library(MASS)

tab_model(glmer_ps, glmer_ps2)
Anova(glmer_ps2)
```
...
comp_ps <- 
  glmer_ps %>% 
  avg_comparisons(variables = list(educationf = c("comp_school", "University"), incomef = c("low", "high")), by = c("annee_f", "canton"))
...
comp_ps %>% 
  ggplot()+
  aes(x = as_numeric(annee_f), y = estimate, color = contrast, shape = contrast)+
  geom_point()+
  geom_pointrange(aes(ymin = conf.low, ymax = conf.high))+
  theme_bw()+
  geom_hline(yintercept = 0)+
  facet_wrap(~canton)+
  scale_y_continuous(labels = scales::percent)+
  labs(title = "Difference in predicted probabilites",
       x = "", y = "")
