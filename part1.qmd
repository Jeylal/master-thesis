...
The two main explanatory variables are income and education levels. The latter's variable is centered around zero to make the intercept meaningful and is treated as continuous for simplicity. The income variable needed more recoding since the scale of the variable changed in 1991: "nivmena" is a four scale variables on the respondent's standard of living, while "revenu" is a more standard income variable in five categories. In order to have significant and robust results, years are grouped into categories of two.
...
summary(glmer_pss <- glmer(data = vox,
                           formula = pss ~ (1| canton_f/annee_cat),
                           family = binomial(link = "probit")))

summary(glmer_pss <- glmer(data = vox, formula = pss ~ income + educ + age_cat + gender + (income + educ + gender | canton_f/annee_cat), family = binomial(link = "probit"),
...
pred %>% 
  ggplot()+
  aes(x = annee_cat, y = estimate, color = term)+
  geom_point()+
  geom_pointrange(aes(ymin = conf.low, ymax = conf.high))

pred_inc_pss %>% 
  ggplot()+
  aes(x = annee_cat, y = estimate)+
  geom_point()+
  geom_pointrange(aes(ymin = conf.low, ymax = conf.high))
