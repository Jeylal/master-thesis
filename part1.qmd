...
2.  *Bourgeois bloc hypothesis*
    ...
    The support for the right increases with both income and education levels. Popular groups support either the nativist far-right (SVP) or abstention.
...
The main dependent variables are dummy variables for party identification and political leaning. Some variables regarding issue position on public policies such as redistribution, state intervention and equal rights between Swiss citizen and foreigners are also considered.

The main independent variables are education and income levels. The variable "educ" is a qualitative variables on the highest achieved educational formation with 6 modalities ranging from primary school to university. It is important here to stress some peculiarities of the Swiss educational system. Switzerland is one of the countries in which vocational training is the most important. As a result, other education categories tend to be missing for a lot of year-periods of the dataset. To solve this problem, but also to simplify the data structure, years are aggregated into seven time-periods.
...
@fig-educ shows the proportions of each level of education and their evolution over time. In the 1980s, most of the Swiss individuals completed either vocational training or compulsory school. The share of compulsory schooling declined over time, reflecting the overall increase in education, as seen also by the increase in the share of university graduates. The share of vocational training declined over time, but is still the most widespread education level in Switzerland. For the regression analysis, the categories "university" and "higher specialized school" are merged together since the absence of the latter in the first two time periods could cause estimation issues. This does not pose a major issue since higher specialized schooling is rather similar than university in Switzerland.

Regarding income levels, the dataset contains two variables. The variable "nivmena" represents the houshold's standard of living in four categories (high, middle-high, middle-low, low) available from 1981 to 1991 whereas "revenu" is an income bracket variable in five categories available from 1992 to 2016. To harmonize these two variables in order to construct one single income-standard of living variable, I recode the variable "revenu" in order to make it similar to "nivmena" by grouping the income brackets categories 3 (from 5'001 chf to 7'000 chf) and 4 (from 7'001 to 9'000) into the "middle-high" category. More information about the recoding done for the regression analysis is available in the appendix.
...
#| echo: false
#| label: fig-inc
#| fig-cap: "Income levels in Switzerland: 1981-2016"
vox %>% 
  group_by(annee_cat) %>% 
  count(incomef) %>% 
  mutate(prop = n/sum(n)) %>% 
...
A series of binary logit regressions for party support, political leaning and some issue positions are run. The results presented in this section come essentially from the following model:
...
One possible methodological issue of the VoxIt dataset is the clustering structure of the data. On the one hand, the observations (Swiss citizen) are clustered into cantons, which are an important level in Switzerland. On the other hand, the time structure and trend of the data are highly relevant for our research question. The cantons represent an important institutional and political level in Switzerland, and it thus likely that individuals within the same canton are not independent and show some patterns and correlation. To assess this potential issue, multilevel regression with years and cantons as levels were conducted. However, the results (see appendix) of the multilevel regression show that the intra-class correlation coefficient are low for the cantonal level, indicating that there is not much difference between cantons. Moreover, the results of the multilevel regressions and @eq-1 are very similar.
...
reg_left <- glm(data = voxlr, 
                left ~ incomef*educationf + incomef:annee_cat + educationf:annee_cat + annee_cat + gender + age_cat + religion + regionL+ married + house_ownership, family = binomial(link = "logit"))

reg_center <- glm(data = voxlr, 
                center ~ incomef*educationf + incomef:annee_cat + educationf:annee_cat + annee_cat + gender + age_cat + religion + regionL+ married + house_ownership, family = binomial(link = "logit"))

reg_right <- glm(data = voxlr, 
                right ~ incomef*educationf + incomef:annee_cat + educationf:annee_cat + annee_cat + gender + age_cat + religion + regionL+ married + house_ownership, family = binomial(link = "logit"))

#########3 issue position regression
reg_state <- glm(data = voxlr, 
                stateint ~ incomef*educationf + incomef:annee_cat + educationf:annee_cat + annee_cat + gender + age_cat + religion + regionL+ married + house_ownership, family = binomial(link = "logit"))

reg_redistr <- glm(data = voxlr, 
                redistribution ~ incomef*educationf + incomef:annee_cat + educationf:annee_cat + annee_cat + gender + age_cat + religion + regionL + married + house_ownership, family = binomial(link = "logit"))

reg_equal_rights_chforeign <- glm(data = voxlr, 
               equal_rights_chforeign  ~ incomef*educationf + incomef:annee_cat + educationf:annee_cat + annee_cat + gender + age_cat + religion + regionL + married + house_ownership, family = binomial(link = "logit"))
